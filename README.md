...
下面的 `distance_band` 已统一为 3/6m 口径：`<3m = Close`，`3-6m = Careful`，`>6m = Safe`。

| 方案 | 检测/距离来源 | Eval 口径 | 距离 MAE | RMSE | 0.5m 内 | 1.0m 内 | distance_band |
|---|---|---:|---:|---:|---:|---:|---:|
| 原方案一：base YOLO + distance head | 官方 `yolo11n.pt` 特征 + distance head | 135 / 135 | 0.554m | 1.629m | 82.2% | 88.9% | 97.8% |
| 新方案一：fine-tuned YOLO + distance head | work-zone YOLO 特征 + 新 distance head | 135 / 135 | 0.428m | 1.088m | 84.4% | 93.3% | 90.4% |
| 新方案二：fine-tuned YOLO + MoGe + calibration | work-zone YOLO + MoGe + calibrator | 134 / 135 | **0.379m** | **0.983m** | **89.6%** | **95.5%** | **95.5%** |
...
同一批 500 图的新 VLM 对比结果：

```text
artifacts/workzone_v1/results/qwen3_vl_flash_all500_ft_head_summary.json
artifacts/workzone_v1/results/qwen2_5_vl_3b_local_all500_ft_head_summary.json
artifacts/workzone_v1/results/qwen3_6_flash_vlm_direct_distance_all500_summary.json
```
...
说明：
- 属性-only 的三行使用同一个检测和距离结果，差异主要来自 VLM 对 PPE / helmet / orientation / occlusion 的判断。
- `qwen3.6-flash` VLM 直接估距这一行不使用本地 distance head 的距离，而是让 VLM 看图直接报米数，所以 distance MAE 明显更高。
- API 首轮按 `batch=50` 跑，超时 batch 用 `batch=25, timeout=600s` 补跑后再合并；本地 3B 用 `batch=1`，其中 3 张图重试后仍为空 JSON，按 `uncertain` 计入结果。
